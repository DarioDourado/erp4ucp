...
@php
    $selectedFamily = trim((string) old('product_family', $product->family ?? ''));
    $selectedUnit   = trim((string) old('product_unit', $product->unit ?? ''));
    $selectedTax    = trim((string) old('taxRateCode_Product', $product->taxRateCode ?? ''));
    $imagePath      = !empty($product->image) ? asset($product->image) : url('upload/no_image.jpg');
@endphp
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form method="post" action="{{ route('product.update') }}" id="myForm" enctype="multipart/form-data" novalidate>
                            @csrf
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="form-group col-sm-10">
                                    ...
                                    <small class="text-muted d-block mt-1">
                                        Se não escolher nova imagem, a imagem atual será mantida.
                                    </small>
                                </div>
                            </div>
                                    ...
                                    <div class="row align-items-start">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <select id="product_family"
                                                ...
                                                @foreach($families as $f)
                                                    @php $familyValue = trim((string) $f->family); @endphp
                                                    <option value="{{ $familyValue }}"
                                                        {{ $selectedFamily === $familyValue ? 'selected' : '' }}>
                                                        {{ $f->family }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <select id="product_unit"
                                                ...
                                                <option value=""></option>
                                                @foreach($units as $u)
                                                    @php $unitValue = trim((string) $u->unit); @endphp
                                                    <option value="{{ $unitValue }}"
                                                        {{ $selectedUnit === $unitValue ? 'selected' : '' }}>
                                                        {{ $u->unit }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <select id="product_taxRateCode"
                                                    ...
                                                    class="form-select select2 @error('taxRateCode_Product') is-invalid @enderror"
                                                    data-placeholder="Selecione...">
                                                <option value=""></option>
                                                @foreach($taxRates as $t)
                                                    @php $taxValue = trim((string) $t->taxRateCode); @endphp
                                                    <option value="{{ $taxValue }}"
                                                            data-description="{{ $t->descriptionTaxRate }}"
                                                            data-rate="{{ $t->taxRate }}"
                                                            {{ (string) $selectedTax === (string) $taxValue ? 'selected' : '' }}>
                                                        {{ $t->taxRateCode }}
                                                    </option>
                                                @endforeach
                                            </select>
...
$(document).ready(function () {
    ...
    function updateTaxDescription() {
        let selected = $('#product_taxRateCode option:selected');
    ...
    $('#product_taxRateCode').on('change', function () {
        updateTaxDescription();
    });
